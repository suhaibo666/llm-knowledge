...
| 推理框架 | [[02_engineering/03_infer_frameworks/index]] | 32 | 活跃 |
| └─ vLLM | [[02_engineering/03_infer_frameworks/vllm/index]] | 24 | 活跃 |
...
> 页面数为递归统计（含各级 index，不含 SUPERSEDED 存根）；推理框架与 vLLM 于 2026-08-30 随 vLLM `6b110bad` 全域复审重新统计为 32/24 页；训练框架与 torchtitan 于 2026-08-27 随 TorchTitan `a3168782c` 机制级复审重新统计为 67/24 页；后训练框架于 2026-08-28 随 verl `254a23ed` 全域复审增至 47 页，其中 `verl/` 15 页、`slime/` 21 页；其余工程域沿用最近一次域级统计。
