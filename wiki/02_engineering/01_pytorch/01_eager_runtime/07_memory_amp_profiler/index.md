---
title: "07 · 运行时:缓存分配器 / AMP / Profiler — 目录索引"
---
...
> **段位与阅读顺序**(kb-reorg P4 Task 9.5,2026-07-30):段 0(01-09)入门;段 1(10-19)核心机制。本目录本地内容页仅 2 篇,一篇一段;首行 `courses/torch_compile_end_to_end` 是外部课程页的交叉引用,不物理位于本目录,不参与编号(2026-07-30 起该课程页取代原 `19_.../00_pytorch_graph_series_index`)。

| 页面 | 层次 | 核心主题 |
|------|------|---------|
| [[courses/torch_compile_end_to_end]] | **系统主线**(外部交叉引用) | 从 FX/AOT saved activation 进入 Inductor logical buffer、Scheduler last-use、wrapper reuse，并严格区分 runtime allocator 的 physical block/segment |
...
- [[courses/torch_compile_end_to_end]] — 编号化端到端课程：卷 D–E 连接 wrapper、内存、CUDAGraph、观测与性能
