---
title: "02 · torch.distributed 原生原语 — 目录索引"
---
...
除了上述「就地」的传统集合 API,PyTorch 另有一套 **functional collectives**(`torch/distributed/_functional_collectives.py`):不原地改输入、返回新张量,实际下发到 `torch.ops._c10d_functional.*` 算子,返回值包成 `AsyncCollectiveTensor`(首次真正使用前自动 `wait`)。这让通信对 [[02_compile_stack/01_dynamo/index]] / [[02_compile_stack/04_inductor/index]] 可见、可被编译器做通信代数优化与计算重叠调度。这是 DTensor / TP 能与 `torch.compile` 协同的关键,细节见本模块 deepdive。
...
- [[01_eager_runtime/06_nn_module_system/index]] — DDP/FSDP 都包裹 `nn.Module`,在 Module 树上挂 autograd hook、改写参数为分片;理解参数/buffer 注册表是前提
- [[01_eager_runtime/01_tensor_and_storage/index]] — DTensor 是 `torch.Tensor` 子类,分片即对底层 storage 的切片与 `resize_`;张量/存储语义是基座
- [[02_compile_stack/01_dynamo/index]] — functional collectives 让通信对 Dynamo 图捕获可见,从而进入编译流水
- [[02_compile_stack/04_inductor/index]] — 编译期对通信算子做调度/重叠优化(comm-compute overlap)的承接方
- [[01_pytorch/index]] — 本域总索引

## Related Pages

- [[courses/torch_compile_end_to_end]] — 编号化端到端课程：卷 F 的 DDP、FSDP、DTensor 与分布式图边界
...
- [[02_compile_stack/01_dynamo/index]] — 图捕获:functional collectives 的可编译入口
- [[02_compile_stack/04_inductor/index]] — 通信-计算重叠的编译期优化
