---
title: "torch.fx / torch.export / 扩展机制 — 源码级深析"
---
...
- **Dynamo**:经 CPython PEP-523 帧求值钩子在字节码层捕获,能处理控制流(靠 guard + graph break)。本页不展开,详见 [[02_compile_stack/01_dynamo/index]]。
...
- `run_decompositions`(`torch/export/exported_program.py:1501`)做进一步 ATen 分解(decomp table),这条链与 [[02_compile_stack/02_aot_autograd/index]] 的分解栈同源。
...
**为什么**:只有进了分发器,自定义 kernel 才能被 autograd、`torch.compile`、export、FX 正确处理。`custom_op` 包裹的函数对编译/追踪是 **不透明黑盒**(docstring 明确:"Preventing torch.compile/export/FX tracing from peeking inside your function" `torch/_library/custom_ops.py:82`),从而避免被错误内联或被 trace 进去看到不该看的内部实现。分发器底座见 [[01_eager_runtime/02_dispatcher_and_device/index]]。
