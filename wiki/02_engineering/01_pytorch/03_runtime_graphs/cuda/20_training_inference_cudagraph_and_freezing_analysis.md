---
title: "F08 · Training、Inference、Freezing 与 CUDA Graph 的组合边界"
---
...
> 最后更新：2026-07-30(kb-reorg P4 Task 6 随 D06 一并迁入本目录,去 f08_ 前缀;判重 vs [[10_pytorch_cuda_graphs_complete_guide]] 方式2/综合比较节——本页 freezing/training-inference 边界内容 Guide 未覆盖,保留为专题页)
...
本页对应卷级入口 `tools/labs_torch_compile/demo_f_advanced_topics.py` 的 `inference_freezing_cudagraph` 用例。默认以 CUDA 为验收设备：

```powershell
python -B tools\labs_torch_compile\demo_f_advanced_topics.py `
  --case inference_freezing_cudagraph --device cuda `
  --output-dir tools\labs_torch_compile\artifacts\volume_demos\f08
```

先用 `--list --json` 查看用例声明的能力要求。无 CUDA 的机器可把 `--device` 改为 `cpu` 探索设备无关机制；CUDA/Triton/多卡专属用例会返回 `BLOCKED`，且不会执行用例正文。不要把 `BLOCKED` 写成 `PASS`。

重点读取 `summary.json` 与 `inference_freezing_cudagraph/result.json`：`status` 区分 `PASS/BLOCKED/FAIL`，`environment` 固化运行环境，`observations` 保存本页机制的实测字段，`artifacts` 指向图代码、日志、trace 或进程证据。`PASS` 只表示该次运行中的断言通过，不外推到其他 PyTorch 版本、shape、dtype 或硬件。

## Related Pages

- [[28_aotinductor_packaging_and_deployment_analysis]]
- [[20_activation_checkpoint_recompute_and_compile_analysis]]
- [[11_cudagraph_trees_warmup_record_and_replay_analysis]]
