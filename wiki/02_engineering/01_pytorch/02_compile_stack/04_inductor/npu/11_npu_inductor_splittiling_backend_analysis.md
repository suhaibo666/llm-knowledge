---
title: "torch.compile 路径分析（一）：Inductor Triton 路径"
---
...
> 对照**实验性 Linearize 方案**（独立包 `npu_inductor_2.9.0`，import 时与本后端注册互斥）见 [[23_npu_inductor_linearize_backend_analysis]]；三方 output code 逐行对比 + 实测对标见 [[31_npu_inductor_linearize_vs_builtin_comparison]]。
...
- [[10_npu_inductor_backend_analysis]] — NPU Inductor 后端集成架构
- [[10_npu_inductor_backend_analysis]] — NPU 后端内部机制
- [[20_npu_lowering_guide]] — NPU 特定 lowering 步骤与算子映射
- [[12_npu_compile]] — NPU 编译工作流
- [[15_inductor_compile_fx_orchestration_analysis]] — PyTorch Inductor compile_fx 编排入口
- [[30_triton_vs_mlir_backend_analysis]] — Triton vs Torch-MLIR 后端对比
- [[21_npu_inductor_optimization_analysis]] — NPU Inductor 优化思想全景（硬件特性 → 思想 → 案例，本页「what/how」的「why」侧互补）
