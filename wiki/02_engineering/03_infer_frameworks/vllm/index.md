...
> **最后更新**:2026-06-22(新建:11 篇源码级分析 + 本索引)
> 一套 11 篇 vLLM V1 推理引擎源码级分析,按用户视角的 **调度 → 模型库 → 特性优化** 三支柱组织,每篇以 **Overview → Quick Start → Deep Dive** 三个维度由浅入深展开。所有非平凡论断均带 `file.py:line` 出处(基准 `485bbe1c6`)。范式对标训练框架 [[torchtitan/index]] / [[megatron-lm/index]]:从"系统怎么搭、机制怎么实现、性能怎么压"逐层拆解。
...
| **算子融合 + 分段 CUDA Graph** | decode 阶段 CPU 下发瓶颈、小算子开销 | CustomOp 多实现 + torch.compile 融合 Pass + 注意力切出、其余录图 replay | [[vllm_fused_ops_and_kernels_analysis]] · [[vllm_compilation_cudagraph_analysis]] |

这四点贯穿全部 11 篇:调度支柱讲前两点的"机制",模型库支柱讲第三点的"计算侧",特性优化支柱把第四点与投机/量化/并行/融合一起展开。
...
| [[vllm_scheduler_analysis]] | 调度算法 | 连续批处理(token 级)、`schedule()`(`scheduler.py:388`)先 running 后 waiting、token 预算"追赶"模型、分块预填充、抢占/重算、优先级 |
...
**连续批处理 vs 静态批处理**(详见 [[vllm_scheduler_analysis]]):V1 把 prefill 与 decode 统一成"让 `num_computed_tokens` 追赶 `num_tokens_with_spec`"的单一过程,每步产出 `{req_id: num_tokens}` 字典——分块预填充只是 `num_new_tokens = min(剩余 prompt, token 预算)` 的自然结果,无独立代码路径。
...
**算子融合的三件事**(详见 [[vllm_fused_ops_and_kernels_analysis]]):`CustomOp` 给一个算子多份实现按平台/开关选(Inductor 下默认走 native 让其自动融合)→ `PostGradPassManager` 把一组 pattern-match 融合 pass 挂进 Inductor `post_grad_custom_post_pass`,塌成单 kernel → `fused_moe` 用 grouped GEMM 避免逐专家小 GEMM,块大小由 autotune config 决定。
...
- **想读懂"为什么快"**:[[vllm_fused_ops_and_kernels_analysis]] + [[vllm_compilation_cudagraph_analysis]] 一起看(算子怎么拼大、下发怎么录图)。
...
- [[megatron_fusion_operators_analysis]] · [[torchtitan_compute_memory_optimizations_analysis]] —— 训练侧融合算子目录,与 [[vllm_fused_ops_and_kernels_analysis]] 的推理侧融合对照
- [[PyTorch_CUDA_Graphs_Complete_Guide]] · [[torch_compile_architecture]] · [[PyTorch_Inductor_Technical_Analysis]] · [[PyTorch_Dynamo_Technical_Analysis]] —— [[vllm_compilation_cudagraph_analysis]] / [[vllm_fused_ops_and_kernels_analysis]] 依赖的底层编译/图捕获/Pattern-Match 栈
...
- 特性优化:[[vllm_feature_optimizations_overview]] · [[vllm_speculative_decoding_analysis]] · [[vllm_quantization_analysis]] · [[vllm_distributed_inference_analysis]] · [[vllm_compilation_cudagraph_analysis]] · [[vllm_fused_ops_and_kernels_analysis]]
