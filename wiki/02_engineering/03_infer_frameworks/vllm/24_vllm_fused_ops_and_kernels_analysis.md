---
title: "vLLM 融合算子与 Kernel：在稳定语义下选择设备实现"
---
...
## 三、`CustomOp` 同时服务 eager、compile 与平台派发
...
1. **`CustomOp` 的 `enforce_enable` 是过渡机制。** 注释自陈：`# This enforce_enable mechanism will be removed after we adding a separate compilation_config for multi-modal part.`，见 `vllm/model_executor/custom_op.py:181-185`。方向是多模态部分拿到独立的 compilation config，而不是靠单个 op 强制开启来绕过全局开关。
