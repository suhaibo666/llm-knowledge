...
> 与原生 `torch.compile` 的关系:vLLM 并没有另造一套编译器,它仍然调标准 `torch.compile`,只是把 `backend` 换成自己的 `VllmBackend`,从而在 Dynamo 抓到 FX 图之后接管"图怎么切、怎么编、怎么包 CUDA Graph"。Dynamo / Inductor 栈本身见 [[02_compile_stack/01_dynamo/index]] / [[PyTorch_Inductor_Technical_Analysis]] / [[torch_compile_architecture]]。
...
- [[torch_compile_architecture]] · [[PyTorch_Inductor_Technical_Analysis]] · [[02_compile_stack/01_dynamo/index]] —— torch.compile 栈
