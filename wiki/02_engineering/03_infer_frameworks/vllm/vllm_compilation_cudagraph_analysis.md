...
> 与原生 `torch.compile` 的关系:vLLM 并没有另造一套编译器,它仍然调标准 `torch.compile`,只是把 `backend` 换成自己的 `VllmBackend`,从而在 Dynamo 抓到 FX 图之后接管"图怎么切、怎么编、怎么包 CUDA Graph"。Dynamo / Inductor 栈本身见 [[PyTorch_Dynamo_Technical_Analysis]] / [[PyTorch_Inductor_Technical_Analysis]] / [[torch_compile_architecture]]。
...
## Cross-Domain Links
- [[PyTorch_CUDA_Graphs_Complete_Guide]] —— CUDA Graph 原理与捕获/replay
- [[torch_compile_architecture]] · [[PyTorch_Inductor_Technical_Analysis]] · [[PyTorch_Dynamo_Technical_Analysis]] —— torch.compile 栈
- [[torch_compile_npugraphs_deep_dive]] —— NPU 图捕获对照
