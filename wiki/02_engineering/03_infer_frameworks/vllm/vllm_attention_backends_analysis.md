...
>
> 本页回答:模型里的一个 `Attention` 层在 V1 引擎里到底做了什么(把 K/V 写进**分页** KV cache + 调所选后端做注意力),后端是怎么按平台/dtype/特性选出来的,以及连接「调度 + KV 管理」与「kernel」的 `AttentionMetadata` 桥是怎么搭起来的。内存侧(块如何分配/回收/前缀复用)归 [[vllm_kv_cache_management_analysis]];Q/K/V 投影、RoPE 等层内算子归 [[vllm_model_library_analysis]];本页只讲**注意力抽象层 → metadata → 后端 → kernel** 这条链路,以及 PagedAttention / FlashAttention / FlashInfer / Triton / MLA 五类后端的分工。
...
flowchart TD
    subgraph SCHED["调度 / KV 管理 (每步)"]
        SO["SchedulerOutput<br/>(vllm_scheduler_analysis)"]
...
| `block_table_tensor` | `backend.py:419` | `(B, max_blocks)` 每请求的逻辑块→物理块映射 | [[vllm_kv_cache_management_analysis]] 的 BlockTable |
...
**关系小结**:
- 与 [[vllm_kv_cache_management_analysis]]:`block_table` 与 `slot_mapping` 都源自块管理器分配的物理块——metadata 是块管理结果的"张量快照"。
- 与 [[vllm_scheduler_analysis]]:`SchedulerOutput` 决定了本步哪些请求、各跑多少 token、各自上下文多长,直接喂出 `query_start_loc`/`seq_lens`/`num_*`。
...
一致性由 selector 兜底:选定后端后,`_cached_get_attn_backend` 读 `backend.get_required_kv_cache_layout()`(`selector.py:133` → `backend.py:377`),若后端要求特定布局就全局 `set_kv_cache_layout(...)`(`selector.py:135-142`、`utils.py:112`)。`indexes_kv_by_block_stride()`(`backend.py:204`)进一步判断"num_blocks 是否物理最外维",决定能否跨层统一分配 / 页大小填充。这套布局协商保证了 [[vllm_kv_cache_management_analysis]] 分配的物理块能被选中的 kernel 正确解读。
...
## Related Pages
- [[vllm_kv_cache_management_analysis]] · [[vllm_model_library_analysis]] · [[vllm_scheduler_analysis]] · [[vllm_feature_optimizations_overview]]
- [[vllm/index]] · [[../index]]
