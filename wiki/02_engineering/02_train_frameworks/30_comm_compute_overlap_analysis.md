...
> **Megatron EP 行补注**：`overlap_dispatch_backward_with_experts_wgrad`（`megatron/core/transformer/moe/moe_layer.py:453-461`、`:533-542`，专用 `_delayed_wgrad_stream` 把专家 wgrad 与 dispatch 反向 A2A 重叠）见 [[14_megatron_ep_analysis|EP 训练闭环]]。它与"EP（跨 microbatch）+ PP 打通"行的 combined_1f1b delay-wgrad（`delay_wgrad_compute`）是**两个相互独立、互斥**的机制（`megatron/core/transformer/transformer_config.py:3723-3734`）：前者**不耦合 PP**，纯 EP 内 wgrad-vs-dispatch-A2A 重叠；后者是 combined_1f1b 内**跨 microbatch**的 wgrad 延迟，二选一。
