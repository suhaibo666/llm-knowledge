...
> **范围**:本页只讲"如何把集合通信藏到计算背后"——TP 的 all-gather/reduce-scatter、EP 的 all-to-all、PP 的 P2P、DP 的日志 all-reduce 各自被哪种特性、用什么机制掩盖。**并行的切分结构本身**(为什么要这些通信)归姊妹页 [[mindspeed_parallelism_analysis]],这里只交叉引用、不重复。每条非平凡结论带 `file:line`,行号均经实际打开核对。属 [[mindspeed/index]] 系列。
...
| 被掩盖的通信 | 发生位置 | 掩盖特性 | 机制(软件流水 / 算子融合 / 换调度) |
|---|---|---|---|
| TP all-gather + reduce-scatter | 列/行并行 Linear(SP) | **MC2** | 融合:`npu_all_gather_base_mm` / `npu_mm_reduce_scatter_base` |
| TP all-gather / reduce-scatter / all-reduce | 列/行并行 Linear | **CoC** | 流水:chunk 切分 + 双 stream;或 lcal 融合核 `coc_ops` |
| EP all-to-all(dispatch/combine) | MoE 层 | **moe-alltoall/allgather-overlap** | 流水:异步 a2a/AG/RS handle 与 GroupedMatMul 重叠 |
| EP all-to-all(跨微批) | MoE 层 | **moe-fb-overlap** | 流水:微批 i 前向与微批 i−1 反向交错 + dw 延迟 |
| EP all-to-all-v | MoE 层 | **moe-alltoall-mc2** | 融合:`npu_alltoallv_gmm` / `npu_gmm_alltoallv` |
| PP P2P(send/recv) | 流水级边界 | **optimize-p2p / optimize-send-recv** | 换流:独立 isend/irecv + 专用通信 stream |
| PP 气泡(bubble) | 流水 warmup/cooldown | **DualPipeV** / **RiPipe** | 换调度:双向切半 / 用气泡做重计算 |
| DP 日志 all-reduce | train_step 末尾 | **async-log-allreduce** | 异步 Work,延迟到取 loss 值时才 wait |
...
### 2.1 命题
序列并行(SP)下,**列并行** Linear 前向要先沿序列维 `all-gather` 输入、再做 $XA^\top$;**行并行** 前向做完 $XA^\top$ 再 `reduce-scatter`。串行执行 = 计算时间 $T_c$ + 通信时间 $T_m$。MC2 不在 Python 层拆分,而是调用昇腾 `torch_npu` 的**单个融合大算子**,在算子内部把 GEMM 的 tile 与 HCCL 集合通信交错排布——一个 NPU op 内通信几乎全被 GEMM 掩盖,省去 launch / 同步 / 显存往返开销。

### 2.2 机制:四个 `autograd.Function` 走前/反向
补丁把 Megatron 的 `ColumnParallelLinear`/`RowParallelLinear` 整类替换为 `MindSpeedMC2{Column,Row}ParallelLinear`(`mc2.py:39-42`),前置硬性要求 `TP>1` 且 SP 打开、与 CoC 互斥(`mc2.py:23-28`)。核心是哪个 TP 集合通信被融进哪次 GEMM:

| 路径 | 融合算子 | 融合的 TP 通信 | file:line |
|---|---|---|---|
| 列并行 **前向** | `npu_all_gather_base_mm` | AG(序列)+ matmul | `linear_function.py:40-48` |
| 列并行 **反向** | matmul 求 dX,async `_reduce_scatter_base` 与算 dw 重叠 | RS(dX) | `linear_function.py:99-101` |
...
列并行前向把"沿首维 all-gather 输入 + 矩阵乘"合成一次调用:
...
`hcomm_info` 是从进程组取出的 HCCL 通信域名(`linear_function.py:7-20`);`gather_output=(not all_gather_recomputation)` 决定是否多吐一份聚合输入——若开重计算就不吐、反向再 AG 一次省显存(`linear_function.py:76-87`)。**列并行反向**是经典"通信发出→算 dw→等通信"软流水:async 发 RS(dX) 后,先算 grad_weight,最后 `reduce_scatter_work.wait()`(`linear_function.py:99-101,149`)。
...
### 2.4 算法:被掩盖比例
设 GEMM 计算 $T_c$、TP 通信 $T_m$。串行 $T_c+T_m$;融合后理想 $\max(T_c,T_m)$。被掩盖比例

$$\eta=\frac{\min(T_c,T_m)}{T_c+T_m}\quad\xrightarrow{T_c\approx T_m}\ 0.5$$

即 $T_c\approx T_m$ 时省下接近一半通信墙钟。

### 2.5 与 DP 梯度重叠的解耦协同
反向算 dw 时,若 `gradient_accumulation_fusion` 且 `main_grad` 为 fp32,直接调融合算子把 dw 原地累加进 `weight.main_grad`(`npu_matmul_add_fp32`,`linear_function.py:110-112`),省一次显存往返;同时返回**占位** grad_weight 并置 `grad_added_to_main_grad=True`,保证 `overlap_grad_reduce=True` 时反向 hook 都跑在主反传线程、不在后台线程触发,避免 DP 梯度 reduce 与权重 hook 的死锁(`linear_function.py:114-135` 注释)。**MC2(TP 掩盖)与 DP 梯度重叠是分层解耦、可叠加的**。

### 2.6 tradeoff(MC2 vs CoC)
| 维度 | MC2 | CoC |
|---|---|---|
...
### 3.1 命题
与 MC2"靠融合算子"不同,CoC 在 **PyTorch 层**把 GEMM 沿 m(token)维切成 `parallel_num` 个 chunk,逐块发**异步**集合通信,使第 $i$ 块的通信 handle 与第 $i\pm1$ 块的 `torch.matmul` 重叠——纯脚本即可达到与 MC2 类似的掩盖,且可移植/可调粒度。另有一条用昇腾 **lcal** 库融合核的快路径。

### 3.2 机制:`COCParallel` 的双流水序
...
```python
# coc_utils.py:200-216 —— run_compute_first(行并行 / REDUCE_SCATTER 走这条)
for i in range(self.split_num):
    compute_output = self.compute_fcn(input_slice)   # 先算第 i 块 matmul
    work, _ = self.comm_fcn(i, compute_output)       # 再对其结果发异步通信
    self.works.append(work)
# 循环末统一 wait —— 通信藏在"下一块的计算"后

# coc_utils.py:221-248 —— run_communicate_first(列并行 / ALL_GATHER 走这条)
for i in range(self.split_num):
    work, output_i = self.comm_fcn(i, input_)        # 先发第 i 块异步通信
    if pre_output is not None:
        pre_work.wait()                              # 当本块通信在飞时…
        self.compute_fcn(input_tensor=pre_output,    # …计算第 i-1 块
...
### 3.3 时间线(`run_communicate_first`,parallel_num=4)
...
### 3.4 算法:切几块才划算
$p$ 个 chunk,每块计算 $T_c/p$、通信 $T_m/p$。流水稳态每步 $\max(T_c/p,T_m/p)$,加一次填充:

$$T_{\text{CoC}}\approx \max(T_c,T_m)+\frac{\max(T_c,T_m)}{p}=\max(T_c,T_m)\Big(1+\tfrac1p\Big)$$

$p\uparrow$ 越逼近下界 $\max(T_c,T_m)$,但每块子 GEMM 越小、越易跌出昇腾 cube 高效区。故 CoC 切块前可选调 `get_aligned_mm_inputs` 把 m/k 对齐 512B,反向若 `grad_output` 内轴非 512B 对齐则 `t().contiguous().t()` 重排内存(`coc_parallel_linears_sequence_parallel_fused.py:51-52`,`ALIGN_SIZE=512`,`:11`)。`get_parallel_num` 若判 m 太小直接退化为不切的 `Rewrite*`(`coc_utils.py:73-82`)。

### 3.5 lcal 融合核快路径
开 `--coc-fused-kernel` 时改走 `mindspeed.ops.lcal_functional.coc_ops`:列并行 `coc_ops.all_gather_matmul(input_, weight, output, bias)`(`coc_parallel_linears_sequence_parallel_fused.py:35`)、行并行 `coc_ops.matmul_reduce_scatter(...)`(`:111`)——这是昇腾 lcal(低延迟集合通信库)在 kernel 内做的 matmul+collective 融合,思路同 MC2 但走 lcal 而非 `torch_npu`。`parallel_num∈{2,4,8}` 时退回非融合的 `COC*SeqParallelFunction` chunk 流水、`==1` 退 `Rewrite*`(`:23-26`)。

> **CoC vs MC2 一句话**:同是"通算重叠",MC2=单融合大算子(粒度不可调、必须 SP),CoC=软件 chunk 流水(粒度可调、SP/非 SP 皆可)或 lcal 融合核;二者互斥,二选一。

---

## 4. MoE 通算重叠 —— 把 EP all-to-all 藏到专家 GEMM 背后

MoE 瓶颈是 token dispatch/combine 的 **EP all-to-all**。MindSpeed 四条互斥路线都把 a2a 做成异步 handle,与 GroupedMatMul / 共享专家 / 对侧微批计算重叠。统一异步原语在 `overlap/comm_utils.py`:`async_all_to_all`(`:217-254`)、`async_all_gather`(`:161-185`)、`async_reduce_scatter`(`:188-214`)都返回 `(tensor, handle)`,可选在独立 `COMM_STREAM` 上发起(`comm_utils.py:173-176`)。

### 4.1/4.2 alltoall-overlap / allgather-overlap

前向编排 `MoELayerOverlapAllToAll.forward`(`moe_layer_overlap_all2all.py:33`):router(`:44-49`)→ `token_permutation`(内部发异步 dispatch a2a,`:84-86`)→ 专家 GroupedMatMul(`:88`)→ `token_unpermutation`(combine a2a,`:90`)。专家 GEMM 与 a2a 的重叠在 `GroupedMlpWithCompAndCommOverlapAll2All`,反向里先对 permute 梯度发异步 a2a 拿 handle、**立刻**做 GroupedMatMul 反传、最后才交棒 handle:
...
...
# :226 紧接着算 op_dx(GroupedMatMul 反传),a2a 在飞行中被吃掉
...
# :309-316 op_dw / op_gmm_add 算专家权重梯度
```

`allgather-overlap`(dispatcher=allgather)对偶:对 TP×EP 输入发 `async_all_gather` 拿 `ag_handle`,在飞时算 `op_dx`,再发 `async_reduce_scatter`,然后 `ag_handle.wait()`——只是把 a2a 换成 AG/RS。**共享专家**(若有)放 `ctx.moe_layer.shared_experts.stream` 这条独立 stream,与路由专家 a2a 并行(`moe_layer_overlap_all2all.py:379`)。
...
让**一个微批的前向层**与**另一个微批的反向层**同时执行,使前向的 dispatch a2a 与反向的 combine a2a / GEMM 互填空隙。实现核心是四个组合函数(前向×反向 ∈ {dense,moe}²),`overlap_funcs/fwdbwd.py`:`…dense_backward_moe…`(`:37`)、`…moe_backward_dense…`(`:381`)、`…dense_backward_dense…`(`:684`)、`…moe_backward_moe…`(`:845`)。两个关键招式:

**(1) dx/dw 解耦**——`WeightGradStore` 包住反传,只先算 grad_input(dx),把权重梯度 dw 入队延后:

```python
# fwdbwd.py:203-205 —— 只算 dx,dw 入队
...
dw 对后续层无数据依赖,可拿去填后续气泡。**(2) 共享专家独立 stream**——反向共享专家的计算放 `bwd_dispatcher.overlap_stream`(`fwdbwd.py:191-194`),与路由专家 a2a 并行。
...
**算法直觉**:单微批串行 = $T_{\text{attn}}+T_{\text{gemm}}+T_{a2a}$;跨微批交错后,微批 $i$ 的 $T_{a2a}$ 藏进微批 $i{-}1$ 的 $T_{\text{gemm}}/T_{\text{attn}}$,dw($\approx$ 半个 GEMM 反传)被推去填通信尾部空隙,稳态每层 $\approx\max(T_{\text{comp}},T_{a2a})$。约束:`EP>1 & ETP=1`、grouped-gemm、调度须 {无PP/VPP/DualPipeV} 之一,与 alltoall-overlap、`overlap_grad_reduce`、`swap_attention` 互斥。

### 4.4 alltoall-MC2 —— 融合 a2a-v + GroupedMatMul
与 §4.1–4.3 的"异步 handle 软流水"不同,这里用昇腾**专家 MC2 融合算子**把 all-to-all-v 与 GroupedMatMul 编进单 kernel:前半段调 `torch_npu.npu_alltoallv_gmm`(dispatch a2a-v + 专家 FC1,`mc2_fuse_a2a.py:39`),后半段调 `torch_npu.npu_gmm_alltoallv`(专家 FC2 + combine a2a-v,`:130`);反向对偶。约束严格:`alltoall_seq`→TP=1、`alltoall`→ETP=1 且仅 dropless,与上面三条及 `use_ascend_mc2` 全互斥。
...
| allgather-overlap | 异步 AG/RS handle | 同微批 op_dx,RS 跟随 | 否 | 中 | `comm_utils.py:161-185,188-214` |
...
直觉:前三条是"流水掩盖"(异步 handle / 跨微批交错),最后一条是"融合掩盖"(单 kernel);fb-overlap 暴露最低但约束最重(EP>1 & ETP=1 + 调度受限),alltoall-MC2 与所有软流水路线互斥。

> EP all-to-all 的语义/切分与跨框架对照见 [[mindformers_moe_token_dispatcher_analysis]] 与 [[mindspeed_parallelism_analysis]]。
...
`DualpipeVFeature`,`--schedules-method dualpipev` + `--dualpipev-dw-detach`("detach dw in cooldown to reduce bubble",`dualpipev_feature.py:17`)。把 Megatron 的 `forward_backward_pipelining_without_interleaving` 整体替换为 `forward_backward_pipelining_with_cutinhalf`,并改 `get_num_layers_to_build` 让**每个 PP rank 持 2 个模型 chunk**(`dualpipev_feature.py:70-75`)——这是 DeepSeek DualPipe 的 V(cut-in-half)变体:模型与数据都成对。`generate_dualpipev_schedule` 把每 rank 的时间轴排成 **7 段**:
...
**算法**:overlap 稳态区长度 $\propto$ 微批数($\text{num\_overlap}=2m-4P+2i+2$,随 $m$ 线性增),而 warmup/cooldown(气泡)只 $\propto P$(`warmup=2P-2-2i`)。于是气泡占比 $\sim O(P)/O(m)$,且双向半流水把单向 $p{-}1$ 的填充级数砍半 → 等效流水深度约减半。约束:需 `untie_embeddings_and_output_weights`、`PP>1`、`num_layers≥2·PP`、微批 `≥2·PP−1`,与 CP/VPP/swap-attention/custom-fsdp/`overlap_grad_reduce`/tp_2d 互斥(`dualpipev_feature.py:20-52`)。

### 5.2 RiPipe(用气泡做重计算)
`RiPipeSchedulesBubbleFeature`(`--recompute-in-bubble`)/`RiPipeSchedulesAdvanceFeature`(`--recompute-in-advance`)。**命题**:VPP 交织 1F1B 的 warmup/steady 本就有空闲气泡,RiPipe 把"全量重计算"的反传重算挪进这些气泡——**既省激活显存又不增加端到端时间**(等于免费的重计算)。实现 `forward_backward_ripipe_pipelining` 在稳态 1F1B 里用 `should_recompute(fk)` 精确判定哪些微批的重算放进哪个气泡:
...
**算法**:只要单微批重算耗时 $T_{\text{recomp}}\le$ 气泡时长 $T_{\text{bubble}}$,端到端不变而省下该微批的激活显存;`get_ripipe_recompute_count_params` 据 PP/VPP/warmup 算出可塞进气泡的重算微批数。`recompute-in-bubble` 仅支持 VPP 交织且依赖 `overlap_p2p_comm`;`recompute-in-advance` 提前重算缩短关键路径。与 `optimize_send_recv_comm` 互斥。

### 5.3 optimize-p2p-comm / optimize-send-recv-comm
两者都让 PP 的 send/recv 不再阻塞计算。**optimize-p2p-comm** 极简——只把 `config.batch_p2p_comm = False`:
...
### 6.1 命题(一条重要澄清)
这里掩盖的**不是**梯度 all-reduce(那由 distributed-optimizer 的 `overlap_grad_reduce` 负责),而是 **train_step 末尾对 loss 标量做的日志 all-reduce**。跨 DC(数据中心)训练时这次小通信延迟很高,改成异步、延迟到真正打印 loss 时才 `wait`,即可把这段网络往返藏到优化器 step 背后。

### 6.2 机制
`AsyncLogAllreduceFeature` O2,`--async-log-allreduce`,补丁替换 `train_step`。`losses_reduced` 变成 `list[tuple(dict, Work)]`(`async_log_allreduce.py:158`),取值前才 wait:

```python
# async_log_allreduce.py:56-63 —— lazy wait
val = x[0][key]
handle = x[1]
if not isinstance(handle, torch.distributed.Work):
    raise AssertionError(...)
handle.wait()          # 真正取 loss 值的那一刻才等异步 all-reduce
return val
```
...
**算法**:被掩盖比例 $\eta=\dfrac{\min(T_{\log},T_{\text{opt}})}{T_{\log}+T_{\text{opt}}}$;跨 DC 下 $T_{\log}$(高延迟小包)被 $T_{\text{opt}}$ 完全吸收,关键路径几乎不见这次往返。
...
| moe-alltoall-mc2 | seq→TP=1 / alltoall→ETP=1、dropless | mc2、alltoall/allgather-overlap、fb-overlap |
...
**一句话总结**:TP 用 MC2(融合算子)/CoC(软件流水)把 AG/RS 藏进 GEMM;EP 用 alltoall/allgather-overlap、fb-overlap(跨微批 + dw 延迟)、alltoall-MC2 把 a2a 藏进专家 GEMM;PP 用 DualPipeV(双向切半缩气泡)、RiPipe(气泡做重计算)、optimize-p2p(独立 isend/irecv)消 P2P 与气泡;DP 用 async-log 把日志 all-reduce 藏进 optimizer.step。
