...
> 配套阅读:`tp_analysis.md`、`ddp_optimizer_analysis.md`、`parallelism_orchestration_analysis.md`
> 定位:"第一层补遗"第③份。补齐 3 块前面只点了名的内容。先做一处**勘误**:tier-1 清单里把它们叫"非均匀 TP / 弹性 resharding",读源码后更准确的命名是 ——

| tier-1 旧称 | 准确含义 |
|------------|---------|
| Megatron-FSDP 内部 | ZeRO-2/3 的**具体实现机器**(`ddp_optimizer_analysis.md` 只给了概念) |
...
### 1.1 动机:`ddp_optimizer_analysis.md` 只讲了概念
...
`megatron_fsdp.py:106`。包住模型,按 `data_parallel_sharding_strategy` 选 ZeRO 阶段(`no_shard`/`optim`/`optim_grads`/`optim_grads_params`,对应 ZeRO-0/1/2/3,见 `ddp_optimizer_analysis.md` §0.3)。一个 `TrainingState` 状态机(`:63`)跟踪"此刻参数/梯度处于分片还是聚合态"。
...
- **HSDP**:`outer_dp_sharding_strategy`(内层节点内分片、外层跨节点复制),见 `ddp_optimizer_analysis.md` §3。
- **接入**:`mcore_fsdp_adapter.py` 把 Megatron-FSDP 缝进 Megatron-LM(`--use-megatron-fsdp`)。

### 1.6 与 DDP 文档的衔接

`ddp_optimizer_analysis.md` 的阶段③④(`optim_grads`/`optim_grads_params`)说"由 Megatron-FSDP 实现" —— 实现就是本节:`ParamAndGradBuffer` + `AllGatherPipeline`(预取重叠)+ `GradReducePipeline` + 临时桶分配器。ZeRO-3 通信 1.5× 不掉速,全靠这套预取流水线。
...
| 主题 | 何时关心 | 与已有文档的关系 |
|------|---------|-----------------|
| **Megatron-FSDP 内部** | 用 ZeRO-2/3(`--use-megatron-fsdp`) | 是 `ddp_optimizer_analysis.md` 阶段③④的实现机器:预取流水线 AG/RS + 临时桶分配器 |
| **Nonuniform TP** | 千卡级长训练要扛 TP 组单点故障 | `tp_analysis.md` 的容错扩展;非侵入式子类 |
| **Resharding / Refit** | RL 训练,训练模型↔推理模型布局不同 | 独立工具,跨 `parallelism_orchestration_analysis.md` 描述的两套布局搬权重 |

至此"第一层补遗"3 份文档全部完成:① 并行编排 capstone、② PP 补遗、③ TP·FSDP·resharding 补遗。

---

*生成依据:`Megatron-LM` `dev` 分支 `ee3f1ff`。源码行号以该 commit 为准。配套文档:`tp_analysis.md`、`ddp_optimizer_analysis.md`、`parallelism_orchestration_analysis.md`、`pp_supplements_analysis.md`。*

## Related Pages

- [[tp_analysis]] · [[ddp_optimizer_analysis]] · [[parallelism_orchestration_analysis]] · [[rl_posttraining_consistency_analysis]] · [[dist_checkpointing_analysis]]
