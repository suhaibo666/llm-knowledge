...
> 配套阅读:`parallelism_orchestration_analysis.md`、`ddp_optimizer_analysis.md`、`tp_fsdp_resharding_supplements_analysis.md` §3(resharding)
> 定位:模型/优化器状态如何**存盘与读取**。
...
| | **dist_checkpointing**(本文) | **resharding / refit**(`tp_fsdp_resharding_supplements_analysis.md` §3) |
|--|------------------------------|--------------------------------|
...
- **`flattened_range`**:DDP 的扁平梯度 buffer、分布式优化器的扁平状态(见 `ddp_optimizer_analysis.md`)—— 局部片是大扁平 buffer 的一段,用这个字段定位。
...
前面文档见过的例子:`ColumnParallelLinear.sharded_state_dict`(`tp_analysis.md`,权重按输出维切→沿 axis 0 分片)、`RowParallelLinear.sharded_state_dict`(沿 axis 1)、`DistributedOptimizer.sharded_state_dict`(`ddp_optimizer_analysis.md`,优化器状态按 DP 切→用 `flattened_range`)、`TEGroupedMLP` 的专家用 `prepend_axis_num` 表达专家轴。
...
> Megatron-FSDP 另用 `fsdp_dtensor` 格式(基于 DTensor,见 `tp_fsdp_resharding_supplements_analysis.md` §1.5)。
...
*生成依据:`Megatron-LM` `dev` 分支 `ee3f1ff`。源码行号以该 commit 为准。配套文档:`parallelism_orchestration_analysis.md`、`ddp_optimizer_analysis.md`、`tp_fsdp_resharding_supplements_analysis.md`。*

## Related Pages

- [[parallelism_orchestration_analysis]] · [[ddp_optimizer_analysis]] · [[tp_fsdp_resharding_supplements_analysis]]
