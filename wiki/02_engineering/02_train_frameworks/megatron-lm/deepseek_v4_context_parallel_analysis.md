...
> **关键结论**：MLA 的 KV 压缩使得 V4 在 CP 场景下的通信量相比标准 MHA 降低两个数量级（~1/128）。这是 V4 能够支持超长序列（1M+）训练的核心原因之一——即使 CP size = 8，每层的 CP 通信量也仅为 `~4 × 7/8 × B × 1M × 64 bytes ≈ 224 MB × B`，在 NVLink 上可在毫秒级完成。
