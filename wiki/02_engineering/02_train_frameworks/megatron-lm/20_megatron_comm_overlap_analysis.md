...
> **主题**：训练里说「通信被掩盖了」时，具体是谁掩盖谁。本页逐轴给出 TP、CP、EP、PP、DP、FSDP 六条掩盖对——掩盖物是什么、在哪里收口、什么条件下失效、代价是什么，并说明为什么没有统一的 overlap 框架、TP 的掩盖对为何按列并行与行并行分成两套、同一个 `overlap_p2p_comm` 在前向和反向掩盖的东西为何不同；随后把六条轴放进一个 step 的跨轴时间线，讲 PP×DP 这个唯一的公共对齐接口、进程级 `CUDA_DEVICE_MAX_CONNECTIONS` 的三方争用与另外三处交界，最后给出从「分支有没有激活」走到「关键路径缩短了多少」的五层诊断梯子。
> **适用范围**：六条掩盖路径的配对关系与组合行为，以及本页拥有的 `align_grad_reduce`、`grad_sync_func` 两个配置实体；各轴的本地实现、通信量推导与配置全表见 12–16 各页与 [[36_megatron_fsdp_analysis]]，`overlap_p2p_comm_warmup_flush` 见 [[15_megatron_pp_schedulers_analysis]]，跨框架的掩盖对比见 [[30_comm_compute_overlap_analysis]]。
