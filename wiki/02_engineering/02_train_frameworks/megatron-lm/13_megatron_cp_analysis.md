...
> **主题**：上下文并行怎样把一条长序列切到多张卡上，以及这条数据面有多少落在 Megatron 自己的源码里。本页先用一个 $c=4$、$S=16$ 的例子讲 zigzag 切分，再讲标准 attention 的接入面：`cp_comm_type` 这条可按层取值的字符串如何解析、四种取值怎样连同 CP 进程组与 `cp_stream` 走到 TransformerEngine、原生 all-gather 回退实现每一步做什么；随后展开线性 attention 的 `linear_cp_mode` 与 Mamba-2 的 `MambaContextParallel` 两条非标准数据面，以及分层 CP 进程组构造、`cp_partition_mode` 布局转换和 Dynamic CP 的换组位置。核心代码在 `megatron/core/transformer/` 与 `megatron/core/ssm/`。
> **适用范围**：Megatron 侧三类层的 CP 接入面与配置面；CP 的通用机制（首尾配对为什么均衡、因果块裁剪、四种调度的算法推导与通信量代数）见 [[20_ring_attention_and_context_parallel_analysis|Ring Attention 与 CP 通用机制]]，packing 与动态 CP 流水线见 [[29_megatron_packed_dataset_dynamic_cp_analysis]]，DSv4 案例见 [[35_deepseek_v4_context_parallel_analysis]]。
