...
> 配套阅读:`megatron_pp_schedulers_analysis.md`、`megatron_ep_analysis.md`、`megatron_tp_analysis.md`、`megatron_cp_analysis.md`、`megatron_ddp_optimizer_analysis.md`
...
给定 order="tp-cp-ep-dp-pp",一张 global_rank 的卡同时属于:
  ...
  DP 组   ← 梯度 all-reduce / ZeRO 分片域(megatron_ddp_optimizer_analysis.md)
...
| `megatron_ddp_optimizer_analysis.md` | `dp`/`dp_cp` 组、HSDP 的内外层 ← `get_ranks("dp")` / `setup_process_groups_for_ddp` |
