...
> **主题**：怎样让存档时的并行度和读档时的并行度互不知情也能对上。本页用「`TP4×DP2` 存、`TP2×DP1` 载」这一个例子讲清楚：每个 rank 用描述子声明自己那一片是全局张量的哪一块（四型描述子与模型侧四种分片惯用法）、校验为什么检的是 chunk 网格上的访问计数，然后展开三条互不影响的变体轴——格式后端、把写盘读盘摊到组内的 fully-parallel wrapper（四条排序键与三条 exchange 数据面）、异步存档的完成阶梯，最后走一遍 `save` 与 `load` 的真实步骤并结算开销。核心代码在 `megatron/core/dist_checkpointing/`。
> **适用范围**：`megatron/core/dist_checkpointing/` 全子系统与落盘语义，含 `CheckpointConfig` 完整配置契约；RL 训推之间不落盘的 GPU↔GPU refit 见 [[30_megatron_rl_posttraining_consistency_analysis]]，分片优化器状态本身的构造见 [[16_megatron_distributed_optimizer_analysis]]，rank 几何见 [[17_megatron_parallelism_orchestration_analysis]]，`fsdp_dtensor` 格式见 [[36_megatron_fsdp_analysis]]，本地/非持久 checkpoint 的容错语义见 [[27_megatron_job_resilience_analysis]]。
