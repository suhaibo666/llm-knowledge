...
> 覆盖分布式训练框架、并行策略、通信优化
> 最后更新: 2026-09-03（Megatron-LM 重构为三页入门、Dense 主线与七个问题分支；机械复算本域 76 页）
...
| 框架 | 在本域中的定位 | 本库覆盖 | 基线 |
|---|---|---|---|
| **Megatron-LM** | NVIDIA 出品，**手工并行的事实标准**；5D 并行、MoE、通信掩盖、分布式优化器的原始参考实现 | 35 篇 + index（**系统性源码覆盖 + 代码仓功能树对账 + 阅读路径**） | `NVIDIA/Megatron-LM@85902ef59`（`dev`，2026-09-01） |
...
| 目录 | 核心主题 |
|------|---------|
| [[megatron-lm/index]] | NVIDIA Megatron-LM；三页入门、Dense 主线、七个问题分支与 A–Q 功能树，共 35 篇内容页 + index（`dev` 85902ef59，2026-09-01 基线） |
...
> **20 号编号空出**(2026-08-01,spec §3.4 补执行):`20_megatron_pp_parallelism_analysis.md` 的 PP 调度增量已并入 `megatron-lm/15_megatron_pp_schedulers_analysis.md`；同批删除的 `megatron-lm/26_megatron_pp_supplements_analysis.md` 中，packed/dynamic CP 的现行权威分析归 `megatron-lm/29_megatron_packed_dataset_dynamic_cp_analysis.md`。`20` 号不重新分配,详见 `wiki/changelog.md`。
