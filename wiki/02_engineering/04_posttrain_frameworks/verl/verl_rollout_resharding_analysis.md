...
- **输入/输出契约**:`generate_sequences` 收 `DataProto`(见 [[verl_dataproto_analysis]]),返回 `DataProto`。HF 基线返回 `prompts/responses/input_ids/attention_mask/position_ids`(`workers/rollout/hf_rollout.py:162-171`),naive 基线额外返回 `old_log_probs`(`workers/rollout/naive/naive_rollout.py:106-116`)。
...
`__enter__/__exit__` 切换全局 SP 进程组(`ulysses.py:373-380`)。这与权重重分片正交——它解决的是"FSDP 沿 DP 切数据、Ulysses 要求 SP 组内数据一致"的矛盾(见 [[verl_workers_engine_analysis]]),不是 train↔generate 的权重搬运。
...
- [[verl_architecture_overview_analysis]] —— hybrid/colocated/standalone 整体架构,本页是其"权重搬运"子系统
- [[verl_ray_trainer_analysis]] —— PPO 主循环如何调度 `update_weights` / `sleep_replicas`(`ray_trainer.py:1645-1672`)
- [[verl_workers_engine_analysis]] —— `ActorRolloutRefWorker` / TrainingWorker / `get_per_tensor_param` 训练引擎侧细节
- [[verl_single_controller_analysis]] —— `RayWorkerGroup` / Dispatch 如何把 `update_weights` 派发到各 worker
- [[verl_dataproto_analysis]] —— `generate_sequences` 的 `DataProto` 输入/输出契约
- [[verl_optimization_analysis]] —— sleep/wake、offload、IPC、bucket 等显存/通信优化合集
