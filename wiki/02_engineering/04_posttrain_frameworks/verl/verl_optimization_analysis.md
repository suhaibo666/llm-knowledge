...
>
> on-policy RL 的吞吐由 **rollout 长尾** 与 **train/generate 显存拉锯** 共同主导。本篇是 verl 性能/显存手段的**横切目录**:每个机制给一个代码锚点 + "为什么",不重复 3D-HybridEngine 的重分片细节(见 [[verl_rollout_resharding_analysis]])。
...
verl 是 HybridFlow 单控制器 + 多 worker 的 RLHF 框架(架构见 [[verl_architecture_overview_analysis]] / [[verl_single_controller_analysis]])。一个 PPO/GRPO step 串起 **rollout(生成)→ old_log_prob → ref/critic → advantage → actor/critic update** 五段;其中:
...
两层收益:**显存**——推理权重不另开一份全量副本,借训练分片原地 all-gather;**通信**——重组只在必要的并行轴上发生,不走 host 往返。完整的 sharding manager 状态机、`__enter__/__exit__` 时机、与 offload 的交互见 [[verl_rollout_resharding_analysis]]。本篇只需记住:**它是 colocated 模式下 train↔generate 切换不爆显存的前提**。
...
- [[verl_rollout_resharding_analysis]] —— 3D-HybridEngine 重分片细节(本篇 §2 只摘要,机制全在此)
- [[verl_ray_trainer_analysis]] —— PPO 主循环装配、`init_workers`、resource pool 接线
- [[verl_workers_engine_analysis]] —— FSDP/Megatron 引擎 worker、offload 与 micro-batch 执行
- [[verl_single_controller_analysis]] —— 单控制器 / WorkerGroup / dispatch,placement 的运行时基础
- [[verl_dataproto_analysis]] —— DataProto 与 batch 重排/分发的数据载体
- [[verl_rl_algorithms_analysis]] —— GRPO/PPO 与 advantage(prefix-grouper/uid 与之相关)
- [[verl_architecture_overview_analysis]] —— 总体架构与本目录定位
