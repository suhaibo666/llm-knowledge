...
机制细节(WorkerGroup 如何建、`DataProto` 如何在 driver/worker 间流动、resharding 怎么做)见 [[verl_ray_trainer_analysis]] 与 [[verl_rollout_resharding_analysis]]。
...
- [[verl_architecture_overview_analysis]] —— HybridFlow 整体架构与 single-controller 设计
- [[verl_ray_trainer_analysis]] —— `RayPPOTrainer.fit()` 主循环逐步源码分析(本文第 5 节的展开)
- [[verl_single_controller_analysis]] —— single-controller / WorkerGroup RPC 编排
- [[verl_dataproto_analysis]] —— `DataProto` 数据载体
- [[verl_workers_engine_analysis]] —— FSDP / Megatron worker 引擎
- [[verl_rollout_resharding_analysis]] —— rollout 后端与训练↔推理权重 resharding
- [[verl_rl_algorithms_analysis]] —— GRPO / PPO / 优势估计器与 KL 控制
- [[verl_optimization_analysis]] —— 性能与显存优化旋钮
