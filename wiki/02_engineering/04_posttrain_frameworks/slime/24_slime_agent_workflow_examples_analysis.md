...
训练器侧收到的是展平后的输入字典：token 行、response 长度、reward、loss mask、rollout id 等；它不接收消息节点、工具事件或沙箱句柄。[`slime/ray/rollout.py:749-797`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/slime/ray/rollout.py#L749-L797) 官方定制文档也把 agent 循环与 Sample 到训练数据的转换定义为不同扩展点。[`docs/en/get_started/customization.md:32-47`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/docs/en/get_started/customization.md#L32-L47) 通用 `Sample`、三层数据约定和嵌套结构的详细说明见 [[12_slime_sample_datasource_analysis]]；本页只解释 agent 逻辑执行如何到达该边界。
...
3. **逻辑执行标识**：一次逻辑执行即使拆成多个训练片段，也只能占一个 rollout 统计单位；`Sample.rollout_id` 的注释就是为 compact/subagent 的同组片段定义这一标识。[`slime/utils/types.py:93-106`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/slime/utils/types.py#L93-L106)
4. **共享节点只训练一次**：一个被多个叶节点共享的模型 response 只能在首个叶节点训练一次，其他叶节点只能把它当作上下文，否则共享祖先会重复贡献梯度。[`slime/agent/trajectory.py:46-82`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/slime/agent/trajectory.py#L46-L82)
...
| 协议适配层 | 会话、进行中的交互轮次、协议转换、SGLang 采样得到的 ids/logprobs | SWE 任务规则与训练批次 |
| `TrajectoryManager` | 每个会话的消息树、生成/仅路由节点、token 偏移与叶节点线性化 | 工具执行与推理服务资源 |
...
3. **扇出去重 ≠ reward 均分**：共享 id 让训练步和 loss 按一次 rollout 统计，reward 数值仍由 agent 工作流决定。[`slime/ray/rollout.py:799-814`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/slime/ray/rollout.py#L799-L814) [`slime/agent/trajectory.py:307-344`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/slime/agent/trajectory.py#L307-L344)
4. **重启沙箱 ≠ 回放逻辑执行**：进程与文件可以隔离重建，但外部 API 副作用和只完成一部分的消息树不会自动恢复；coding-agent 超时路径会丢弃 sid 并输出 ABORTED 占位样本。[`examples/coding_agent_rl/generate.py:270-333`](https://github.com/THUDM/slime/blob/681b3adca54105d5ecd3fb822fa0dc58a427e0f9/examples/coding_agent_rl/generate.py#L270-L333)

## Related Pages

- [[12_slime_sample_datasource_analysis]] — `Sample` 标识、嵌套扇出、展平与训练输入字典的数据约定。
