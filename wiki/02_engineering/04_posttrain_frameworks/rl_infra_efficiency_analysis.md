...
- 而且 sandbox 时间方差极大（100ms 到几分钟，参见 [[rl_sandbox_design_analysis]] 第 5 节）
...
> **原理**：PPO/GRPO 这类算法本来就是 off-policy 友好的（有 importance sampling 做修正），几步的 staleness 在数学上等价于很轻微的探索噪声。Sholto Douglas 在 Claude 4 访谈中：**RL 的主要作用是激活已有知识并组织成解决方案，不是从零学**。所以策略稍微旧一点，激活效果不会差太多。算法基础见 [[ppo_analysis]] / [[grpo_analysis]]。
...
这就是为什么大厂都在这么做——节省的 GPU 时间是天文数字。详细架构图见 [[rl_sandbox_design_analysis]] 第 4 节。
...
异步训练用 staleness 换效率，但近期一些工作（DeepSeek、月之暗面）显示，更严格的 on-policy（同步或者只允许 1 步 staleness）+ 更激进的算法（GRPO、DAPO）反而能拿到更好的最终质量。未来可能是**两条路线并行**：超大规模训练走异步、追求 SOTA 走严格 on-policy。参见 [[dapo_analysis]] / [[grpo_analysis]] / [[gspo_analysis]]。
...
- Sandbox 不行 → 训练 epoch 时间过长，迭代太慢，team 失去 momentum（[[rl_sandbox_design_analysis]]）
- Infra 不行 → 同样算力别人能跑 10x 数据，你只能跑 1x，能力差距迅速拉开（本页）
- Reward hacking 防御不行 → 表面 benchmark 涨了，实际部署翻车，用户失去信任（[[reward_hacking_defense_analysis]]）
...
| 脏活 | 决定的事 | 对应页 |
|------|---------|--------|
| Reward Hacking 防御 | 训出来的是不是你想要的 | [[reward_hacking_defense_analysis]] |
| Sandbox 设计 | 训练能不能稳定跑 | [[rl_sandbox_design_analysis]] |
...
- [[ppo_analysis]] — PPO Loss 与 importance sampling
- [[dapo_analysis]] — DAPO 的动态采样与长尾治理思路
- [[gspo_analysis]] — 序列级 importance ratio，影响 staleness 容忍度
- [[kimi_k1.5_analysis]] — Kimi 长上下文 RL 训练
- [[RL_PPO_Loss_and_GRPO_Analysis]] — PPO/GRPO 源码级实现
