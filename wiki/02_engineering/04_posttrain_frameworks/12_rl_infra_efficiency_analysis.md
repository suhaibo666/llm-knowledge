...
> [!warning] 数据口径说明（2026-08-11 补，引用本页数字前必读）
> 本页正文中的**百分比与倍数绝大多数没有可追溯出处**，性质分三类，引用时请按类处理：
>
> | 类别 | 具体条目 | 如何使用 |
> |---|---|---|
> | **⚠️ 无出处的经验区间** | §1.1 的 GPU 利用率四档（15-25% / 50-60% / 80-90% / 90%+）、§2 优化 3 的「成本/性能优化 30%+」、优化 4 的「端到端缩短 20-30%」、优化 5 的环境池四档（100 / 1000 / 1万 / 10万）、§3.1 的三段占比假设（30%/40%/30%） | **视为量级直觉，不可作为论据引用**。这些数字既无论文出处也无本库实测，来源是访谈与工程常识的综合 |
> | **⚠️ 舆情/转述级** | 「Anthropic 把 environment 池从千级拉到数十万级」「Claude 内部据说已在做 multi-objective RL」等带"据说"的表述 | 仅作线索，**不可当事实** |
> | **✅ 有一手出处** | 优化 6 的 AReaL `staleness_manager.py:80-112`、Kimi K3 §5.3.1 p.21 的 cache-pressure-aware admission | 可正常引用 |
>
> 另：§2 优化 2 的长尾数学**已于 2026-08-11 重写**（原推导把单条轨迹的 p99 当成了 batch 的 $E[\max]$，并把两种不同的截尾口径混为一谈），详见该处。
>
> **本页是本目录被引用最多、也最不可核验的一页**——这个组合本身就是风险。若要把本页结论写进设计文档，建议先按上表降级或补源。
...
> **数学（2026-08-11 重写，原推导有误）**。设 batch 内 $N$ 条轨迹的完成时间独立同分布于 $F$，$X_{(k)}$ 记第 $k$ 小的次序统计量。
>
> - **等全部完成**是 $X_{(N)}=\max$，其典型值对应分位 $F^{-1}\!\big(1-\tfrac{1}{N+1}\big)$。$N=128$ 时约为 **p99.2**，**不是 p99**。重尾分布下 p99.2 可显著高于 p99，所以用 p99 代表"等全部完成"是**低估**。
> - **等最快的 90%** 是 $X_{(\lceil 0.9N\rceil)}=X_{(116)}$，集中在 $F^{-1}(116/129)\approx$ **p90**。
> - 取 p90 = 5 s、p99 = 60 s，比值 $\ge 60/5=12$。**"12×"这个量级站得住，但原文的推导过程不成立**，且误差方向是低估收益。
>
> **另一处必须区分**：「等前 90%」与「发 $N{+}K$ 取前 $N$」不是同一件事。需要 128 条而发 160 条，等的是 160 个里的第 128 个，对应 $F^{-1}(128/161)\approx$ **p80**——比等前 90% 更快，代价是 **25% 的额外 sandbox 开销**。上文 A、B 两小节混用了这两种口径。
>
> **"几乎免费"的说法应当撤回**。真正的代价不是浪费的 CPU（那确实便宜），而是**选择偏差**：被丢弃的永远是最慢的那批，而 coding RL 里最慢的轨迹**系统性地就是最难、最长、工具调用最多的任务**。持续按完成时间截尾，等于在训练分布里系统性删除难题——这会同时抬高 pass@1 曲线和掩盖能力缺口。[[01_posttraining_infra_mechanism_analysis|D05]] §10 已就此警告过（"'吞吐上升'可能只是允许更多旧样本或**丢弃更多难样本**"），本页此前未作回应，现补记。**部署长尾截断时应同时监控被丢弃样本的难度分布**，而不只看 wall-clock。
