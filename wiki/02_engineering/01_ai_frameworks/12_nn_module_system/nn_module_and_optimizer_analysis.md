...
本页是 [[index]] 概念全景与 [[nn_module_quickstart]] 用法之后的源码层。聚焦 7 个锚点最密集的核心机制:`__setattr__` 注册分派、`_apply` 变换流水线、hook 执行编排、`train()` 递归、Lazy 物化、Parameter/Buffer 元类、Optimizer 深水区,以及 `nn.functional` 的无状态解耦。所有 `路径:行号` 均相对 `E:\97-codes\pytorch\pytorch` 根,落笔前逐一打开核实。
...
这套「状态归 Module、计算归 functional」的二分有两重价值:① Module 有清晰的状态边界(参数/buffer 都在注册表里);② 计算是无副作用的纯函数,便于 FX tracing、`torch.func` 函数式变换与编译期联合图构建(见 [[03_aot_autograd/index]])。`F.linear` / `F.batch_norm` 等同此模式。
...
- [[nn_module_quickstart]] — 本模块 quick start:搭模块、遍历、存取、hook、优化器循环的可跑示例
- [[00_tensor_and_storage/index]] — 基座:Parameter / Buffer 都是 `torch.Tensor` 子类,`_apply` 的搬运语义源于此
- [[10_eager_autograd/index]] — 基座:Parameter 作为 autograd 叶子,`_apply` 须保叶子性与 `.grad`;optimizer `step` 在 `no_grad` 内推进
- [[03_aot_autograd/index]] — 下游语境:`nn.functional` 无状态设计利于 FX tracing 与编译期联合图
