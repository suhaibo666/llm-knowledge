...
> 理解 DTensor 拦截算子的机制可参考 ezyang 的 *PyTorch internals*(Tensor subclass / dispatcher),交叉见 [[01_dispatcher_and_device/index]]、[[00_tensor_and_storage/index]]。
...
`torch/distributed/_functional_collectives.py` 提供**函数式**集合通信:`all_reduce`(`:161`)、`broadcast`(`:145`)、`all_gather_tensor`(`:317`)、`reduce_scatter_tensor`(`:349`)等。与 c10d 原地版不同,它们**不修改输入、返回新张量**,因此对 Dynamo/Inductor 可 trace,编译器能做「通信代数优化」(重排、合并、与计算重叠)。交叉见 [[02_dynamo/index]]、[[04_inductor/index]]。
...
- [[01_dispatcher_and_device/index]] — Dispatcher 与 `__torch_dispatch__`:理解 DTensor/AsyncCollectiveTensor 子类拦截算子的底层机制
- [[00_tensor_and_storage/index]] — Tensor/Storage 内部:`FlatParameter._resize_(0)`、wrapper subclass 的存储语义
- [[02_dynamo/index]] — Dynamo 如何 trace functional collectives
- [[04_inductor/index]] — Inductor 对通信算子的代数优化与计算/通信重叠
- [[12_nn_module_system/index]] — `nn.Module` / `nn.Parameter`:DDP/FSDP/TP 改造的对象
