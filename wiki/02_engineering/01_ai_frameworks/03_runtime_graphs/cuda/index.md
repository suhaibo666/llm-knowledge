...
| [[cudagraph_trees_warmup_record_and_replay_analysis]] | **deep dive(源码级)** | `torch/_inductor/cudagraph_trees.py` 机制深挖:`CUDAGraphNode`/`CUDAGraphTreeManager`、按静态输入地址与动态整数 key 分流的多份 recording、warmup→record→replay→fallback 状态机、invariant 检查;与本页方式2/综合比较判重后保留为专题页(独有内容 >50%) |
| [[training_inference_cudagraph_and_freezing_analysis]] | **deep dive(源码级)** | training/inference/freezing/CUDA Graph 四轴组合边界:`freezing.py` 变换链与所有权后果、`cudagraphify` 地址不变式、组合矩阵与失败/回退边界;本页 Guide 未覆盖,保留为专题页 |
