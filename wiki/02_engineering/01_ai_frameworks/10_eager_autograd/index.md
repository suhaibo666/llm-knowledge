...
| 维度 | **eager autograd(本模块)** | **AOTAutograd**([[03_aot_autograd/index]]) |
|------|------------------------------|---------------------------------------------|
...
- [[03_aot_autograd/index]] — **编译期对应物**:前/反向联合图、partition、functionalization(本模块的 `torch.compile` 侧投影)
- [[00_tensor_and_storage/index]] — `Tensor`/`Storage` 与版本计数(`AutogradMeta` 挂载基础、inplace 检测依据)
- [[01_dispatcher_and_device/index]] — Dispatcher:`VariableType`(Autograd key)正是在分发层拦截算子、触发建图
...
- [[03_aot_autograd/index]] — AOTAutograd:编译期反向图
- [[aotautograd_analysis]] — AOTAutograd 深析(joint graph / partitioner)
- [[01_dispatcher_and_device/index]] — Dispatcher 与 Autograd key
- [[00_tensor_and_storage/index]] — Tensor / Storage / 版本计数
