...
> **对照 NPU**：`NPUTritonKernel.should_use_persistent_reduction` **恒 False**（昇腾不支持 persistent reduction），一律走 looped 路径，并重写 `reduction_resize` 的多维槽位；torch_npu 内置后端则**支持** persistent（UB 塞满的单核形态，见 [[npu_inductor_optimization_analysis]] §四）。
...
更长的 reduction 用 cooperative reduction：`program_id(0)` 让给 split-id，各 block 算自己那段，再经 **semaphore + barrier 跨 block 同步**做最终合并（`init_cooperative_reduction`，`triton.py:3210`；配 `CooperativeReductionGrid`，见 [[inductor_gpu_kernel_dispatch_model]] §四）：
...
> **对照 NPU（关键）**：cooperative reduction 依赖**单 kernel 内 cross-block barrier**,**昇腾无此同步原语**。所以 [[npu_inductor_linearize_backend_analysis]] §五的 **r 轴 rsplit** 用「partial 写 workspace + combine 求和」**两个独立 kernel** 实现同样的「沿 reduction 轴跨核」思路,绕开 barrier。
