...
> 本页是 [[npu_inductor_linearize_backend_analysis]] 的动态 shape 分册。哲学：**「编译一次，运行期自适应」**——Linearize 展平 + 固定 `grid[40,1,1]` + group 循环后，只把**真正动态**的 `length`/`divisor` 作为运行时标量实参进签名，静态值折叠 `tl.constexpr`，**同一份 kernel 源码、同一次编译**覆盖所有 runtime shape。Dynamo/sizevars 的符号化前半段（`s0→ks0`、`size_hints`、guard）则**完全继承上游**，见 [[dynamic_shapes_full_analysis]]。
...
内置后端的动态 shape 难点与 4 个改进方向见 [[npu_compile_paths_overview]] §九、[[npu_inductor_optimization_analysis]] §十一；本后端用运行期标量 + kernel 内循环让一份编译产物适应全谱。
...
- `i64→i32` 降型对动态 size 实参同样生效（昇腾无 i64 计算）——大张量溢出风险见 [[npu_inductor_linearize_backend_analysis]] §六。
