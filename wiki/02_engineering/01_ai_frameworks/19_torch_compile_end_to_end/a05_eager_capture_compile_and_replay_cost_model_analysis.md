...
本页对应卷级入口 `tools/labs_torch_compile/demo_a_execution_model.py` 的 `eager_compile_cost` 用例。默认以 CUDA 为验收设备：

```powershell
python -B tools\labs_torch_compile\demo_a_execution_model.py `
  --case eager_compile_cost --device cuda `
  --output-dir tools\labs_torch_compile\artifacts\volume_demos\a05
```
