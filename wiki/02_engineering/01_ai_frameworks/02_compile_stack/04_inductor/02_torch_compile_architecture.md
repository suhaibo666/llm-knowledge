...
> 前两段的细节属于上游模块,见 [[02_compile_stack/01_dynamo/index]] 与 [[02_compile_stack/02_aot_autograd/index]];Inductor 的入口是 `torch._inductor.compile_fx.compile_fx`,被 AOT Autograd 作为 `fw_compiler`/`bw_compiler` 调用。
...
| 阶段 | 一句话职责 | deepdive |
|------|-----------|----------|
| **Decomposition** | 把复杂/复合 ATen 算子拆解为一小撮原语算子,收敛 IR 规模,让后续 lowering 只需处理有限算子集 | 见 [[33_decomposition_passes_guide]] |
| **FX Graph Passes** | 在 FX 图层面做图级优化,分三段:pre-grad(高层重写)、joint-graph(常量折叠/模式匹配)、post-grad(底层融合/设备相关重写) | [[30_pre_grad_passes_guide]] / [[31_joint_graph_passes_guide]] / [[32_post_grad_passes_guide]] |
| **Lowering** | 把 ATen 算子逐一翻译为 **Inductor IR**(`lowerings[target]` 注册表),用 `Pointwise`/`Reduction` 等循环原语表达计算 | [[10_fx_lowering_to_inductor_ir_analysis]] |
| **Scheduler** | 对 IR 节点做依赖分析,决定融合(水平/垂直)、计算顺序、内存规划与缓冲区复用 | [[13_scheduler_dependency_graph_fusion_and_ordering_analysis]] |
| **CodeGen** | 把调度后的 IR 翻译成具体后端代码:Triton kernel(GPU)或 C++/OpenMP(CPU),并生成驱动 kernel 的 Python/C++ wrapper;期间做 autotuning 选最优实现 | [[20_inductor_codegen_analysis]] |

横切关注点:**动态形状(dynamic shapes)** 贯穿上述每个阶段(符号化 size、guard、`ShapeEnv`、XBLOCK 选择等),单列一条全链路 deepdive,见 [[20_symbolic_shapes_guards_and_graph_reuse_analysis]]。
...
最终落盘的是一段可缓存、可直接执行的 kernel + wrapper 代码。想逐行看,用 `TORCH_LOGS=output_code` / `TORCH_COMPILE_DEBUG=1`(见 [[01_inductor_quickstart]] 与 [[02_compile_stack/07_debugging/index]])。

## 6. 由浅入深导航
本模块按「层次」组织,建议路径:

1. **overview(本页)** — 建立全局心智模型。
2. **上手** → [[01_inductor_quickstart]]:最小前向+反向示例、`torch.compile` 参数与 `torch._inductor.config` 速查、怎么看生成代码。
3. **deepdive** —
   - compile_fx 编排入口:[[15_inductor_compile_fx_orchestration_analysis]];后端/IR 深度:[[11_inductor_ir_values_loops_layouts_and_buffers_analysis]]
   - 各阶段:[[10_fx_lowering_to_inductor_ir_analysis]] · [[13_scheduler_dependency_graph_fusion_and_ordering_analysis]] · [[20_inductor_codegen_analysis]] · FX passes([[30_pre_grad_passes_guide]] / [[31_joint_graph_passes_guide]] / [[32_post_grad_passes_guide]])
   - 横切专题:[[20_symbolic_shapes_guards_and_graph_reuse_analysis]]、[[25_unbacked_symint_analysis]]、[[26_flex_attention_analysis]]、调试 [[02_compile_stack/07_debugging/index]]
...
- [[01_inductor_quickstart]] — 上手:最小示例与参数速查
- [[15_inductor_compile_fx_orchestration_analysis]] — deepdive:compile_fx 编排入口(为什么先调用 AOTAutograd、fw/bw compiler 分工)
- [[02_compile_stack/04_inductor/npu/index]] — NPU Inductor 后端(硬件子目录)
