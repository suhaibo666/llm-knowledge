...
> 本页是「GPU dispatch 模型」的基线，补 [[inductor_codegen_analysis]]（codegen 概览）与 [[inductor_codegen_dynamic_shape_analysis]] §3（grid 动态计算）未展开的 **kernel 骨架 + IterationRanges + grid 类型**。它也是 [[npu_inductor_linearize_backend_analysis]] 中 NPU「40-CU group dispatch」所**替换**的对象——对照阅读最清晰。
...
每个维度（x/y/z）各取一个 `tl.program_id(grid_dim)` 映射到 grid 轴；`program_id * BLOCK` 得到块基址，`tl.arange(0, BLOCK)` 是块内偏移，`mask` 滤越界。这是 NPU 后端用固定 `grid[40,1,1]` + `for i in range(group_size)` 所替换的核心（见 [[npu_inductor_linearize_backend_analysis]] §二）。
...
> 这棵树是 upstream 与 NPU 后端**共享**的；区别在于 GPU 直接用 `program_id + arange` 喂它且 mod/div 廉价，NPU 则要把 mod/div 化简成纯仿射并用 group 循环替代 grid（[[npu_inductor_linearize_backend_analysis]] §二的索引线性化）。
...
| `CooperativeReductionGrid` | `:5297` | `program_id(0)` 让给 reduction-split（见 [[inductor_reduction_codegen_deep_analysis]]） |
...
GPU dispatch 模型把「分块、派发、Y/Z 溢出」交给 grid + 硬件调度器，kernel 内无循环；reduction 的跨核同步见 [[inductor_reduction_codegen_deep_analysis]]；autotune 如何选 BLOCK/grid 见 [[inductor_autotuning_analysis]]。NPU 后端因「固定 40 核单维 dispatch」把这些搬进 kernel body（[[npu_inductor_linearize_backend_analysis]]）。
...
- [[scheduler_dependency_graph_fusion_and_ordering_analysis]] — 调度与融合（tiling 在融合中的作用）
- [[npu_inductor_linearize_backend_analysis]] — NPU 用 group dispatch 替换本模型
