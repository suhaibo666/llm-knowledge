...
机制层面的"为什么这么设计"在 [[fx_graph_export_and_custom_ops_analysis]];本模块全景见 [[index]]。
...
- **要端到端编译加速** → `torch.compile`(见 [[02_dynamo/index]])。
...
- `run_decompositions()` 可进一步把图分解到更细的 ATen 算子集(衔接 [[03_aot_autograd/index]] 的分解栈)。
...
- **旧 API**(对照用,不推荐):`torch._custom_ops.custom_op`(`torch/_custom_ops.py:24`)/ 底层
  ...
  分发器背景见 [[01_dispatcher_and_device/index]],算子注册全景见 [[07_op_registration/index]]。
...
- [[fx_graph_export_and_custom_ops_analysis]] — 本模块 deepdive(Proxy 拦截、Node/Graph IR、代码生成、分发桥接源码级深析)
- [[02_dynamo/index]] — Dynamo:字节码帧求值的另一条图捕获路径
- [[03_aot_autograd/index]] — AOT Autograd / 分解栈(`run_decompositions` 衔接处)
- [[07_op_registration/index]] — 算子注册全景(custom_op 与分发器注册)
- [[01_dispatcher_and_device/index]] — 分发器与 device key(custom_op 落地的底座)
