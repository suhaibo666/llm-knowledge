...
| [[custom_backends_and_device_integration_analysis]] | deep dive(专题) | 设备接入 `torch.compile` 的另一层契约:Dynamo backend / Inductor device backend(含 `DeviceInterface`)/ dispatcher-custom-op backend 三者怎样分层组合;2026-07-30 迁入,与本目录 [[privateuse1_device_integration_analysis]]、`04_inductor` 的 [[codegen_extension_guide]] 三方划界(见页头 note) |
