...
- **[[01_dispatcher_and_device/index]] = 运行时路由**:DispatchKey/DispatchKeySet 的优先级与 redispatch“洋葱”、boxed/unboxed 调用机制、设备接入。即“调用时怎么被路由到正确 kernel”。本模块只在生命周期“运行时收尾”处触及它,细节交给 01。
- **[[07_op_registration/index]] = 同一模式的 NPU 特化实例**:NPU op-plugin 如何把昇腾 kernel 通过 `TORCH_LIBRARY_IMPL(..., PrivateUse1, ...)` 注册进同一个 Dispatcher。即本模块通用机制在具体后端上的落地实例。
...
- [[01_dispatcher_and_device/index]] — 运行时分发与设备接入(DispatchKey/KeySet 优先级、redispatch、PrivateUse1),本模块生命周期的“运行时”那一段在此深入
- [[07_op_registration/index]] — 算子注册的 NPU 特化实例(op-plugin 供给侧),本模块通用机制的落地范例
- [[00_tensor_and_storage/index]] — Tensor/Storage 内部表示,算子操作的数据载体
- [[03_aot_autograd/index]] — AOTAutograd 与 functionalization 消费完整变体集(autogen 的下游)
...
- [[01_dispatcher_and_device/index]] — 运行时路由
- [[pytorch_dispatcher_analysis]] — Dispatcher 优先级/redispatch/boxed-unboxed 深析
- [[07_op_registration/index]] — NPU 算子注册特化实例
- [[00_tensor_and_storage/index]] — Tensor/Storage 内部表示
- [[03_aot_autograd/index]] — functionalization 与变体集下游
