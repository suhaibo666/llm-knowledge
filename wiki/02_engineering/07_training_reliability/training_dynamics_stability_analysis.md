...
**结果层**：loss 与其鲁棒统计量。K2-V2 的做法值得抄：对 loss 维护滑动中位数 $m_t$ 与 **MAD**（中位数绝对偏差），用标准化分数 $|loss - m_t| / \text{MAD}_t$ 判异常（对 spike 本身鲁棒，不会被 spike 污染阈值），触发即自动重启并推送告警；**ZClip 对梯度范数做同样的 z-score 异常判据**。
