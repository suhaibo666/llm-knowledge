...
| **MindSpore 自动并行** | sharding 传播 + 双递归/SAPP 策略搜索（见 [[mindspore_compiler_analysis]]）|
...
- [[mindspore_compiler_analysis]] — sharding 传播式半自动并行的代表。
...
- [[mindspore_compiler_analysis]] — MindSpore 自动并行（传播范式）
