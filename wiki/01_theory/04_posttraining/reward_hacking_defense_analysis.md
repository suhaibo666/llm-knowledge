...
> 环境加固本身依赖 [[rl_sandbox_design_analysis]] 中的强隔离 sandbox。弱沙箱意味着 Layer 1 直接失守。
...
只看 outcome 太弱。OpenAI 的 PRM800K 那条线、DeepSeek R1 的 process reward，未来会和 outcome reward 混用，针对 hack 行为做精准识别。算法基础见 [[grpo_analysis]]。
...
| RL Infra 效率 | 训练能跑多大、多快 | [[rl_infra_efficiency_analysis]] |
...
- [[rl_sandbox_design_analysis]] — 同系列，sandbox 是 Layer 1 的工程基础
- [[rl_infra_efficiency_analysis]] — 同系列，环境规模决定 hack 多样性能否被发现
- [[rlhf_foundations_analysis]] — 含 *Scaling Laws for Reward Model Overoptimization* 等基础工作
- [[grpo_analysis]] — GRPO 与 process reward 的算法基础
- [[ppo_analysis]] — PPO Loss 与 reward 信号设计
- [[dpo_analysis]] — DPO 与 reward 信号绕开思路
- [[instructgpt_rlhf_analysis]] — RLHF 三步流程的 reward model 训练
- [[kimi_k1.5_analysis]] — 长上下文 RL 中的奖励设计
