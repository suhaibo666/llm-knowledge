...
> **更新日期**：2026-07-17
> **原理前置**：[[gdn_kda_linear_attention_analysis]]
...
$C$ 是 tile/autotune 选择，不改变 [[gdn_kda_linear_attention_analysis#五、Chunk size 与 t 的关系|逐 token 递推语义]]。不同 GPU、head dim、序列长度与 backend 可能选择不同 $C$；不能把论文伪代码的 64 当作模型超参数写死。
...
这验证的是实现应满足的工程等价性：相同初态、Q/K/V、门与因果边界下，两条路径产生同一语义的所有输出和末态。由于浮点运算重排，bitwise 完全一致不是目标；测试容差为 `ATOL=2e-2, RTOL=1e-2`（`:18-19`）。数学证明见 [[gdn_kda_linear_attention_analysis#5.2 为什么分块后数学上仍等价]]。
...
- [[gdn_kda_linear_attention_analysis]] — 公式、QKVABZ、RNN 与 chunk 数学等价性
- [[kimi_linear_analysis]] — Kimi Linear 模型与实验
- [[kimi_k3_architecture_deepdive]] — KDA 在 Kimi K3 中的结构位置
- [[kimi_k3_infra_deepdive]] — prefix cache、Mooncake 与服务侧瓶颈
