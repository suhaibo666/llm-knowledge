...
这一段属于 RL 基础设施的 slime 框架，本页只点出其**低精度动机**，完整的 slime / 全异步 RL 见 [[24_glm5_agentic_rl_deepdive]]。
...
- **Lightning Indexer**：把 **score 计算 + ReLU + TopK** 三步集成进**单个 kernel**，让 NPU 把**计算与访存重叠**（§5, p21–22）。（Lightning Indexer 是 DSA 稀疏注意力的内容选择器，架构语境见 [[20_glm5_architecture_deepdive]]。）
...
- [[20_glm5_architecture_deepdive]] — 架构（MLA·Muon Split·MTP·DSA；Lightning Indexer 的架构语境）
...
- [[24_glm5_agentic_rl_deepdive]] — slime + 全异步 RL（FP8 rollout / PD 解耦 的完整语境）
