...
| 页面 | 是否真源于论文 | 裁决 | 主要问题 |
|---|---|---|---|
| [[13_deepseek_v4_analysis]] | ✅ 是 | **基本可信，做定点订正** | 基线头过时；FP4 归属应为后训练 §5.2.1；混入臆造的「DualPath 推理框架」小节；个别「顶尖」措辞被 Table 6 反证 |
| [[23_deepseek_v4_cp_analysis]] | ✅ 是（含真实论文引文） | **可信，仅订正章节号** | §3.5.3→§3.4.3、§3.6/§3.6.1/§3.6.2→§3.5/§3.5.1/§3.5.2 等约 13 处位移；基线头 |
| [[24_deepseek_v4_fp4_qat_analysis]] | ✅ 是 | **可信，定点订正** | 出处 §3.7 错误（应 §5.2.1）；旧路径 `raw/05_model_families/`；「三个组件 FP4 量化」应为「两组件 FP4 + 索引分数 BF16」；效果表数字多为未标注推断 |
| [[25_mhc_analysis]] | ✅ 是（源自 mHC 论文 arXiv:2512.24880v2） | **基本无需改动** | 其消融表（27B，59.0/63.0/63.4）来自 **mHC 论文自身实验**，并非 V4 论文，故「V4 论文查无此数」属正常；与 V4 §2.2 公式一致 |
| [[26_deepseek_v4_technical_deepdive]] | 原 ❌（AI 臆造）→ 已重写 | ✅ **已据正式版整页重写**（2026-06-25） | 旧稿：DSA=CSA+HCA 倒置；Highly/Heavily；HCA「10%」应 m′=128；臆造分层调度/MoE 任务路由/DualPath。新稿逐方程引 §2.3 Eq 9–27 |
| [[27_deepseek_v4_implementation_deepdive]] | 原 ❌（AI 臆造）→ 已重写 | ✅ **已据正式版整页重写**（2026-06-25） | 旧稿：专家 128/激活 8；HCA 0.1；Sinkhorn 100；Muon 实为 Adam；INT8；臆造 PCA/DualPath。新稿伪代码逐行锚定 Eq 1–28/Alg 1，常量取自 §4.2.1 |
| [[28_deepseek_v4_architecture_analysis]] | 原 ❌（AI 臆造）→ 已重写 | ✅ **已据正式版整页重画**（2026-06-25） | 旧稿：128 专家/K=8–12/5%·35% 任务自适应/领域命名/O(n log n)/DualPath/缺 MTP。新稿复刻 Figure 2/3/4 + §4.2.1 配置 |
...
- [[13_deepseek_v4_analysis]] — V4 整体架构（Tier-A，已订正）
- [[23_deepseek_v4_cp_analysis]] · [[24_deepseek_v4_fp4_qat_analysis]] · [[25_mhc_analysis]] — 专题（Tier-A / 旁证）
