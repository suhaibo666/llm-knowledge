...
> **主旨**：V4 在 V3 的 Transformer + MTP 骨架上做三处结构升级——用 **mHC**（流形约束超连接）取代朴素残差、用 **CSA/HCA 逐层交错的混合注意力**取代 MLA、用 **Muon** 取代 AdamW（多数参数）；MoE 仍是 DeepSeekMoE（仅微调）。配套深读见 [[deepseek_v4_analysis]]。
>
...
本页整页替换了预发布期 AI 生成的旧图，纠正以下臆造（详见 [[deepseek_v4_audit_report]]）：
...
- **无 "DualPath / SNIC / CNIC" 推理框架**（论文 §3.5 推理框架为异构 KV cache + on-disk KV，见 [[deepseek_v4_cp_analysis]]）。
...
- [[deepseek_v4_analysis]] — V4 整体架构深读（本页结构图的文字底座）
- [[deepseek_v4_technical_deep_dive]] — CSA / HCA / DSA / MLA 机制级对比
- [[deepseek_v4_implementation_details]] — 配置与实现要点
- [[deepseek_v4_cp_analysis]] — Contextual Parallelism 与推理框架（异构 KV cache / on-disk KV）
- [[deepseek_v4_fp4_qat_analysis]] — FP4 QAT（后训练；MoE 权重 + CSA indexer QK 路径）
- [[mHC]] — 流形约束超连接（B_l 双随机约束 + Sinkhorn-Knopp）
- [[deepseek_v4_audit_report]] — 本页所依据的正式版审计报告
- [[deepseek_moe_analysis]] — DeepSeekMoE（共享 + 细粒度路由专家）
- [[deepseek_v3_analysis]] — V3 架构（MLA / FP8 / DualPipe），V4 的对照基线
