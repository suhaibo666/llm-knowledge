...
> **整页重写**: 2026-06-25。旧版为预发布期 AI 臆造的通用伪代码（路由专家写成 128、HCA 写成 `compression_ratio=0.1`、Sinkhorn `max_iter=100`、"Muon" 实为 Adam、量化写成 INT8、臆造 DualPath/PCA-KV/task_classifier 动态-k），已整页废弃；审计见 [[deepseek_v4_audit_report]]。
>
> **本页定位**: V4 把"百万 token 上下文效率"拆进四类机制——CSA/HCA 沿**序列维**把 KV 压成 $1/m$、$1/m'$（再叠 DSA top-k）；mHC 用**双随机矩阵**约束残差混合保证深堆叠稳定；Muon 用**混合 Newton-Schulz** 正交化更新；DeepSeekMoE 把亲和度激活换成 $\sqrt{\mathrm{Softplus}(\cdot)}$ 并对前 3 层 MoE 用哈希路由。本页给每个机制的逐步伪代码并锚定到论文方程，是 [[deepseek_v4_analysis]] 的**实现级补充**；架构综述见 [[deepseek_v4_analysis]]，对比与图示见 [[deepseek_v4_technical_deep_dive]] / [[deepseek_v4_architecture_diagrams]]，mHC/Muon 数学细节见 [[mHC]] / [[muon_analysis]]。
...
把残差流从 $\mathbb R^{d}$ 扩展为 $\mathbb R^{n_{hc}\times d}$（$n_{hc}=4$）。核心创新：把残差混合矩阵 $B_l$ 约束到**双随机矩阵流形（Birkhoff polytope）**，使谱范数 $\lVert B_l\rVert_2\le 1$（非扩张），且该集合对乘法封闭 → 深堆叠稳定（§2.2 原文，p8）。详尽数学与稳定性分析见 [[mHC]]。
...
**改动清单（§2.1, p7；§4.2.2）**：① 亲和度 $\text{Sigmoid}\to\sqrt{\text{Softplus}(\cdot)}$；② aux-loss-free + **序列级**平衡损失（权重 $10^{-4}$，防单序列内极端不均）；③ **前 3 个 MoE 层用哈希路由**（token-ID 哈希）替代 V3 的 dense FFN；④ 移除"路由目标节点数"约束并重设并行策略；⑤ MTP depth 仍为 1。路由原理见 [[deepseek_moe_analysis]]。

---

## 8. 推理：异构 KV Cache（§3.5.1 / §3.5.2, Figure 6, p21–23）—— 简述

混合注意力产生多种 KV，破坏 PagedAttention 假设（cache 策略多样 + kernel 对齐约束），故 V4 自定义两段式布局。深度分析见 [[deepseek_v4_cp_analysis]]。
...
- [[deepseek_v4_analysis]] —— V4 整体架构、规格、评测综述（本页的上层）
- [[deepseek_v4_technical_deep_dive]] —— CSA/HCA/DSA/MLA 机制对比深解析
- [[deepseek_v4_architecture_diagrams]] —— V4 结构图与时间线图示
- [[deepseek_v4_cp_analysis]] —— Contextual Parallelism + 推理异构 KV 的深度分析（§8 的展开）
- [[deepseek_v4_fp4_qat_analysis]] —— FP4/MXFP4 量化感知训练（§5.2.1，后训练技术）
- [[deepseek_v4_audit_report]] —— 本页旧伪代码的逐项审计与订正依据
- [[mHC]] —— 流形约束超连接的完整数学与稳定性分析（§5 的展开）
- [[muon_analysis]] —— Muon 原理与分布式实现（§6 的展开）
- [[deepseek_moe_analysis]] —— DeepSeekMoE 路由与负载均衡（§7 的展开）
- [[deepseek_v3_analysis]] —— 前代架构（MLA、FP8 训练、DualPipe），V4 的对照基线
