...
>
> 本页是 [[deepseek_v4_analysis]] 注意力章节的**专题深潜**：把 V4 的两条注意力分支 **CSA**（压缩 + 稀疏）与 **HCA**（重度压缩、不稀疏）逐方程拆开，并与 **DSA（V3.2 起源的稀疏选择）**、**MLA（V2 起源的低秩潜在 KV）** 做"动机→机制→证据→为何不选替代"的横向对比。核心论点：**V4 的范式从 MLA 的"压缩每 token 的 KV 通道维"转向"在序列维合并 token"（CSA `1/m` / HCA `1/m′`），CSA 再在已压缩的块上套 DSA top-k**；正是这一序列维压缩让百万 token 上下文在工程上可行（§2.3, p9）。
>
> 本页对预发布期 AI 臆造旧稿的**整页重写**（旧稿裁决见 [[deepseek_v4_audit_report]]）。已剔除旧稿全部臆造内容（"DSA=CSA+HCA"倒置、"Highly"误名、HCA"10%"、分层压缩调度、MoE 任务路由、DualPath、O(n log n) 等均不见于论文）。
...
> ④ **MLA 由 DeepSeek-V2 引入**（[[deepseek_v2_analysis]] §MLA），**V4 不使用 MLA**——它是 V4 要替换的基线。
...
| 机制 | 出处 | 一句话 |
|------|------|--------|
| **MLA** | V2 引入（[[deepseek_v2_analysis]]）；V3/V3.2 沿用 | 把每 token 的 KV 压到一个低秩潜在向量 `c_t^{KV}`（通道维压缩），仍对**全部** token 做 dense attention。V4 不用。 |
...
**为何要"两种"而不是一种**：CSA 保精度（轻压 `1/m=1/4` + top-k 选关键块）、HCA 保极致省（重压 `1/m′=1/128`）。把二者**交错**（§2.3, p9）让模型在"信息保真"与"长程效率"之间分层取舍；论文以二者的混合配置作为"使百万 token 上下文在实践中可行"的关键（§2.3 开篇，Fig 3 caption p9）。具体交错策略见 [[deepseek_v4_analysis]]（Flash 前 2 层纯 SWA、Pro 前 2 层 HCA，其余 CSA/HCA 交错，§4.2.1）。
...
2. **FP4 indexer**：lightning indexer 的注意力计算用 **FP4** 精度，加速超长上下文下的注意力算子（p13）。（FP4 QAT 的完整框架——MoE 权重 + CSA indexer QK 路径走 FP4、索引分数另走 FP32→BF16——属后训练 §5.2.1，详见 [[deepseek_v4_fp4_qat_analysis]]。）
...
| **压缩率** | `1/m`，**m=4**（§4.2.1） | `1/m′`，**m′=128**（§4.2.1） | 1（不压序列，只选 top-k） | 序列维 1；KV≈`9·d_h·l`（[[deepseek_v2_analysis]]） |
...
- **压缩维度** —— 论文给出：CSA/HCA 压**序列维**（Eq 12 / Eq 23），MLA 压**通道维**（[[deepseek_v2_analysis]] §MLA）。据此推断：在 1M token 量级，序列维压缩（条目数随上下文亚线性增长）的 KV 收益远超通道维压缩，这正是 V4 弃用 MLA 的根因（呼应 §2.3.4 的 2% 基线对照）。
- **是否稀疏** —— CSA 是稀疏（Eq 17）、HCA 是 dense（§2.3.2 原句"does not employ sparse attention"）、DSA 是稀疏、MLA 是 dense。
- **压缩率** —— 数字均来自 §4.2.1（m=4、m′=128）。MLA 的 `9·d_h·l` 来自 [[deepseek_v2_analysis]]，作 KV 量级对照，非 V4 论文数字。
- **是否用 indexer** —— CSA 的 indexer query 与主注意力 query **共享**同一低秩 `c_t^Q`（Eq 13/14 与 Eq 18，p11）；HCA 无 indexer（§2.3.2）。
- **适用层** —— Flash 前 2 层纯 SWA、Pro 前 2 层 HCA，其余 CSA/HCA 交错（§4.2.1，详见 [[deepseek_v4_analysis]]）。
...
- [[deepseek_v4_analysis]] — V4 总体架构（本页是其注意力章节的专题深潜）
- [[deepseek_v4_audit_report]] — 旧稿裁决与正式版核对（本页据此整页重写）
- [[deepseek_v4_cp_analysis]] — CSA/HCA 的上下文并行（packed sequences、压缩 KV 通信量）
- [[deepseek_v4_fp4_qat_analysis]] — FP4 QAT（indexer QK 路径 + MoE 权重，后训练 §5.2.1）
- [[deepseek_v4_architecture_diagrams]] — V4 架构结构图（补充参考）
- [[deepseek_v4_implementation_details]] — V4 实现要点（补充参考）
- [[mHC]] — 流形约束超连接（残差层，与注意力并列的另一架构升级）
- [[muon_analysis]] — Muon 优化器（Q/KV RMSNorm 取代 QK-Clip 的依据）
- [[deepseek_v2_analysis]] — MLA 起源、KV cache `≈9·d_h·l` 对照
- [[deepseek_v3_analysis]] — 前代（V3/V3.2 沿用 MLA + V3.2 引入 DSA）
- [[deepseek_moe_analysis]] — MoE 路由与负载均衡（V4 affinity 改 `√Softplus`、前 3 层 Hash 路由）
