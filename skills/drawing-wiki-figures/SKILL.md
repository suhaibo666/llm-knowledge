...
**解算型图必须配回归测试**：为解算器写一个把输出锁定到页面所引数值的测试（范例：
`tools/figs/svg/lib/megatron_pp_sim.test.mjs` 把仿真结果与页面 ASCII 调度表逐格锁定，7 例），
纳入 `pytest`/`node --test` 门禁。源码基线推进导致行为变化时，测试先红——提醒同时更新页面、
图与基线声明，而不是三者各自漂移。

**测试必须真的去读那个 `.md` 文件。** 把数字硬编码进测试、再断言它出现在 SVG 里，只锁住了
图的一侧：图对、正文被改成另一个数，测试照样绿。要挡住漂移，测试得 `readFile` 页面本体，
逐个断言图上算出来的量在正文里出现（范例：`megatron_cp_figures.test.mjs` 的
「页面正文引用的数值与图上一致」一例）。这是最容易实现错的一条——写测试时先问自己：
**只改正文、不改图，这个测试会不会红？**

**越界不等于不重叠。** 只校验图元是否超出 viewBox 的断言，挡不住"芯片压在相邻列文字上、
把行首字形裁掉"这类缺陷——它们全在画布内。要么加图元对图元的包围盒重叠断言，要么按 §5
把图**真正渲染出来看**（`chrome-headless-shell` + `puppeteer-core` 可用）。两者都不做，
就等于没检查排版。
...
- [ ] 主/辅路径分级了？
- [ ] 主体占画面 75%–85%，没有大块留白？
- [ ] 图上每个数字、维度、源码符号都能在正文或冻结基线的源码里对上？（图和正文同源，不是各写各的；不要求行号）
- [ ] 图注是短标签，不是整句解释？
